...
PR #195 added the reviewed Private Local-AI Enclave contract and additive Neon capability. Actual readback on 2026-09-19 confirms the lane-bound identity, queue tables and `NOLOGIN` worker role in both UAT and Production with migration checksum `b49fe8d024c279710eb4eb3b45b06e40ffc960a3ec57b21792c63c98514ceef6`. Both lanes contain zero jobs and zero worker heartbeats, so no local-model pilot or private-job processing is active.

Activation remains gated on a healthy UAT VPS heartbeat, lane-specific worker credentials and envelope keys, log/privacy verification, public-safe fixtures, then synthetic private fixtures. Production must remain disabled until that UAT evidence is reviewed. The required flow is scoped private retrieval → local model → schema-validated structured insight → advisor review. Cloud escalation additionally requires data minimization, de-identification, residual-risk review and explicit workflow opt-in; raw PII and raw conversations remain prohibited.
