...
function jobStatusLabel(status: string) {
  ...
  if (["processing", "executing"].includes(status)) return "กำลังดำเนินการ";
...
function JobRow({ job }: { job: OperationsJob }) {
  return <tr className="border-t border-white/10 align-top">
    <td className="px-4 py-4"><div className="text-sm font-medium text-white/90">{job.source === "article-scheduler" ? "ตั้งเวลาเผยแพร่บทความ" : "ส่งโพสต์โซเชียล"}</div><div className="mt-1 max-w-[240px] break-all text-xs text-white/40">{job.objectId}</div></td>
