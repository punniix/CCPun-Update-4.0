...
export default async function LocalAiPage() {
  ...
  return <div>
    ...
    <p className="mt-3 max-w-3xl text-sm leading-6 text-white/70">ดูสถานะงานที่ให้ AI ภายในช่วยเตรียม ผลลัพธ์ทุกชิ้นต้องผ่านคนตรวจก่อนส่งต่อ และหน้านี้ไม่มีคำสั่งแก้หรือเผยแพร่เนื้อหาอัตโนมัติ</p>
    ...
    <section className="mt-6 space-y-4">
      <div><h2 className="text-xl font-semibold">ผลลัพธ์ที่รอคุณตรวจ</h2><p className="mt-1 text-sm text-white/55">อนุมัติหมายถึงให้ workflow อ่านผลลัพธ์ได้เท่านั้น ยังไม่เขียนฉบับร่างและไม่เผยแพร่</p></div>
      {reviews.map((review) => <article key={`review-${review.jobId}`} className="rounded-3xl border border-[#e0c985]/20 bg-[#e0c985]/[0.05] p-5 md:p-6">
