...
export default function CIResult({ result, onEditData, onReset }: CIResultProps) {
  ...
  return <div data-ui="human-centered-ci-result" className="ccpun-calculator-result ccpun-motion-result-reveal">
    ...
    <section className="ccpun-calculator-result-panel" aria-labelledby="ci-recovery-result-title">
      ...
      </> : <p className="ccpun-calculator-result-body">Recovery Reserve ยังเป็น 0 เพราะคุณยังไม่ได้กรอกจำนวนครั้ง/วันหรือค่าใช้จ่ายช่วงพักฟื้น</p>}
    ...
    <div className="ccpun-calculator-result-cta">
      ...
      <a href={CI_LINE_OA_URL} target="_blank" rel="noopener noreferrer" aria-label="คุยกับ CCPun ทาง LINE OA (เปิดในแท็บใหม่)" className="gold-button ccpun-motion-tactile mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 px-6 py-3 sm:w-auto" onClick={() => trackEvent('ci_contact_click', { tool_name: 'ci_planning', cta_location: 'ci_result', contact_channel: 'line', calculator_version: CI_ASSESSMENT_VERSION })}><MessageCircle className="h-5 w-5" aria-hidden="true" />คุยกับ CCPun ทาง LINE OA</a>
    </div>
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <button type="button" onClick={onEditData} className="glass-button ccpun-motion-tactile flex min-h-12 items-center justify-center gap-2"><Edit3 className="h-4 w-4" /><span>แก้ไขข้อมูล</span></button>
      <button type="button" onClick={onReset} className="glass-button ccpun-motion-tactile flex min-h-12 items-center justify-center gap-2"><RefreshCw className="h-4 w-4" /><span>เริ่มใหม่</span></button>
    </div>
  </div>;
}
