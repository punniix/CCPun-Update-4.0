...
const SOURCE_V3_SHA256 = "60a118c686a972373e4673a41603a3ad7b54c313a99628d281258d2860642fa1";
const SOURCE_V3_URL = new URL("./assets/ccpun-line-rich-menu-v3.png", import.meta.url);
...
export async function loadLineRichMenuV3Asset() {
  const source = await readFile(SOURCE_V3_URL);
  const sourceDigest = createHash("sha256").update(source).digest("hex");
  if (sourceDigest !== SOURCE_V3_SHA256) {
    throw new Error("LINE_RICH_MENU_V3_SOURCE_ASSET_DIGEST_INVALID");
  }
  return validateRichMenuAsset(source, LINE_RICH_MENU_V3);
}
