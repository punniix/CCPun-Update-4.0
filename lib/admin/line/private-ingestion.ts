...
import { z } from "zod";
import type { LinePrivateIngestEvent } from "../../line/private-domain";
import {
...
export function resolveLineIngestRuntime(
  ...
  if (variables.CCPUN_NEON_PROJECT_ID?.trim() !== identity.projectId) return null;
  if (variables.CCPUN_NEON_BRANCH_ID?.trim() !== identity.branchId) return null;
  if (variables.CCPUN_NEON_DATABASE?.trim() !== identity.database) return null;
