...
export default async function GrowthDashboardPage() {
  ...
  return <div>
    ...
    <section className="mt-7 rounded-3xl border border-white/10 bg-white/[0.025] p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-semibold">โอกาสจากข้อมูลคำค้น</h2><p className="mt-1 text-sm leading-6 text-white/60">เรียงจากจำนวนการค้นหาและระดับความยากภายในระบบ ไม่ใช่คะแนน Google หรือคำทำนายอันดับ</p></div><Link href="/content/research/" className="inline-flex min-h-11 items-center rounded-xl border border-white/10 px-4 py-2.5 text-sm text-white/70">เปิดข้อมูลงานวิจัย</Link></div>
      {opportunities.length ? <ol className="mt-4 grid gap-3">{opportunities.map((item, index) => <li key={item.id} className="flex flex-col gap-2 rounded-2xl border border-white/10 bg-black/10 p-4 sm:flex-row sm:items-center sm:justify-between"><div><span className="text-xs text-[#e0c985]">ลำดับ {index + 1}</span><h3 className="mt-1 font-medium">{item.keyword}</h3><p className="mt-1 text-sm text-white/55">{item.provider} · ตรวจเมื่อ {new Intl.DateTimeFormat("th-TH", { dateStyle: "medium", timeZone: "Asia/Bangkok" }).format(new Date(item.checkedAt))}</p></div><div className="text-sm text-white/70">โอกาสภายใน <strong className="text-white">{item.score}/100</strong></div></li>)}</ol> : <p className="mt-4 rounded-2xl border border-white/10 p-4 text-sm text-white/60">ยังไม่มีคำค้นที่มีข้อมูลจำนวนค้นหาและระดับความยากครบ หรือยังไม่มีหัวข้อที่ขาดในบทความ ระบบจึงไม่สร้างรายการจากการคาดเดา</p>}
    </section>
  </div>;
}
