...
export const metadata: Metadata = { title: "หลักฐานการคุย" };
...
export default async function LineEvidencePage({
  params,
}: {
  ...
  const displayName = profile?.displayName || `ลูกค้า ${detail.item.customerCode.slice(-8)}`;
  ...
  return (
    <div className="print:bg-white print:text-black">
      <div className="flex flex-wrap items-end justify-between gap-4 print:block">
        <div>
          <Link
            href={`/dashboard/inbox/${leadId}/`}
            className="text-sm text-white/55 hover:text-white print:hidden"
          >
            ← กลับไปหน้าลูกค้า
          </Link>
          <p className="mt-5 text-xs font-semibold tracking-[0.12em] text-[#e0c985] print:text-black">
            หลักฐานการคุย
          </p>
          <h1 className="mt-2 text-3xl font-semibold">{displayName}</h1>
          <p className="mt-2 text-sm text-white/55 print:text-black/70">
            เก็บไว้สำหรับตรวจสอบย้อนหลังเมื่อจำเป็น
          </p>
        </div>
      ...
      <section className="mt-6 rounded-2xl border border-amber-200/20 bg-amber-200/[0.06] p-4 text-xs leading-5 text-amber-50/80 print:border-black/20 print:bg-transparent print:text-black">
        <strong>เกี่ยวกับหน้านี้</strong>
        <p className="mt-1">
          ถ้าลูกค้ายกเลิกข้อความใน LINE ระบบจะทำเครื่องหมายไว้ แต่สำเนาที่เราได้รับก่อนหน้านั้นยังอยู่ในส่วนหลักฐานนี้
          เฉพาะเจ้าของระบบเท่านั้นที่เปิดดูได้
        </p>
        <p className="mt-1">
          รหัสตรวจสอบของแต่ละรายการใช้เทียบว่าข้อมูลในระบบยังตรงกับตอนที่บันทึกไว้
          ไม่ใช่การรับรองทางกฎหมายจากบุคคลภายนอก
        </p>
      </section>

      <section className="mt-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold">รายการที่เก็บไว้</h2>
          <span className="text-xs text-white/45 print:text-black/60">
        ...
        {items.length === 0 ? (
          ...
          <ol className="mt-4 space-y-3">
            {items.map((item) => (
              <li
                key={item.id}
                className={`rounded-2xl border p-4 break-inside-avoid ${
                  item.direction === "outbound"
                    ? "ml-6 border-[#e0c985]/20 bg-[#e0c985]/[0.04]"
                    : "mr-6 border-white/10 bg-black/15"
                } print:m-0 print:mb-3 print:border-black/20 print:bg-transparent`}
              >
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-white/45 print:text-black/60">
                  <span>
                    {item.direction === "inbound" ? "ลูกค้า" : "CCPun"} ·{" "}
                    {lineMessageTypeLabel(item.messageType)} ·{" "}
                    {lineEvidenceSourceLabel(item.sourceKind)}
                  </span>
                ...
                {item.status === "unsent" ? (
                  <p className="mt-2 inline-flex rounded-full border border-rose-300/20 bg-rose-300/10 px-2.5 py-1 text-xs text-rose-100 print:border-black/30 print:bg-transparent print:text-black">
                    ลูกค้ายกเลิกข้อความนี้
                    {item.unsentAt ? ` · ${formatBangkok(item.unsentAt)}` : ""}
                  </p>
                ) : null}
                ...
                <div className="mt-3 whitespace-pre-wrap text-sm leading-6 text-white/85 print:text-black">
                  {item.contentState === "available"
                    ...
                    : item.contentState === "decrypt_failed"
                      ? "ตอนนี้ยังเปิดข้อความนี้ไม่ได้"
                      : "รายการนี้ไม่มีข้อความให้อ่าน"}
                </div>

                <details className="mt-3 border-t border-white/10 pt-3 text-[11px] leading-4 text-white/35 print:border-black/20 print:text-black/55">
                  <summary className="cursor-pointer font-medium text-white/45 print:text-black/60">
                    รายละเอียดตรวจสอบ
                  </summary>
                  <dl className="mt-2 grid gap-1">
                    <div>
                      <dt className="inline font-medium">รหัสรายการ: </dt>
                      <dd className="inline">{item.id}</dd>
                    </div>
                    <div>
                      <dt className="inline font-medium">รหัสตรวจสอบ SHA-256: </dt>
