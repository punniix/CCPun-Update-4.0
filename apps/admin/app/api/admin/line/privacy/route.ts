import { NextResponse } from "next/server";
import { z } from "zod";
import { isSameOriginAdminMutation } from "@/lib/admin/auth-config";
import { getAdminIdentity } from "@/lib/admin/identity";
import { createPrivacyRequest, listPrivacyRequests } from "@/lib/admin/line/business-intelligence";
import { hasAdminPermission } from "@/lib/admin/rbac";

export const dynamic = "force-dynamic";
const headers = { "Cache-Control": "private, no-cache, no-store, max-age=0, must-revalidate", "X-Robots-Tag": "noindex, nofollow, noarchive" };
const bodySchema = z.object({
  requestType: z.enum(["export","delete"]),
  leadId: z.string().uuid().nullable().optional(),
  customerId: z.string().uuid().nullable().optional(),
}).strict();

export async function GET() {
  const identity = await getAdminIdentity();
  if (!identity) return NextResponse.json({ error:"unauthorized" }, { status:401, headers });
  if (identity.role !== "owner" || !hasAdminPermission(identity.role,"settings:read")) return NextResponse.json({ error:"forbidden" }, { status:403, headers });
  const model = await listPrivacyRequests();
  return NextResponse.json(model, { headers });
}

export async function POST(request:Request) {
  const identity = await getAdminIdentity();
  if (!identity) return NextResponse.json({ error:"unauthorized" }, { status:401, headers });
  if (identity.role !== "owner" || !hasAdminPermission(identity.role,"settings:read")) return NextResponse.json({ error:"forbidden" }, { status:403, headers });
  if (!isSameOriginAdminMutation(request.url, request.headers.get("origin"))) return NextResponse.json({ error:"invalid-origin" }, { status:403, headers });
  const body=bodySchema.safeParse(await request.json().catch(()=>null));
  if(!body.success || (!body.data.leadId && !body.data.customerId)) return NextResponse.json({ error:"invalid-request" }, { status:400, headers });
  try {
    return NextResponse.json(await createPrivacyRequest({ requestType:body.data.requestType, leadId:body.data.leadId, customerId:body.data.customerId, actor:identity.actor }), { headers });
  } catch {
    return NextResponse.json({ error:"privacy-request-unavailable" }, { status:503, headers });
  }
}
