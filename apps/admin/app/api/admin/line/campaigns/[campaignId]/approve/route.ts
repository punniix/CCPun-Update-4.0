...
import { approveLineCampaign } from "@/lib/admin/line/campaigns";
...
export async function POST(request: Request, { params }: { params: Promise<{ campaignId: string }> }) {
  ...
  try { return NextResponse.json(await approveLineCampaign((await params).campaignId, identity.actor), { headers }); }
  catch { return NextResponse.json({ error: "campaign-approval-unavailable" }, { status: 503, headers }); }
}
