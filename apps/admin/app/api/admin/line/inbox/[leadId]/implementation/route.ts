...
import { recordLeadImplementation } from "@/lib/admin/line/business-intelligence";
...
const bodySchema = z.object({
  ...
  partnerCode: z.string().regex(/^[a-z0-9][a-z0-9_-]{0,79}$/).nullable().optional(),
...
export async function POST(request: Request, { params }: { params: Promise<{ leadId: string }> }) {
  ...
  try {
    const result = await recordLeadImplementation({
      leadId: leadId.data,
    ...
    return NextResponse.json({ error: "implementation-update-unavailable" }, { status: 503, headers });
  }
}
