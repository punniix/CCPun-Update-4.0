...
import { attributeLeadRevenue } from "@/lib/admin/line/business-intelligence";
...
export async function POST(request: Request, { params }: { params: Promise<{ leadId: string }> }) {
  ...
  try {
    const result = await attributeLeadRevenue({
      leadId: leadId.data,
    ...
    return NextResponse.json({ error: "revenue-attribution-unavailable" }, { status: 503, headers });
  }
}
