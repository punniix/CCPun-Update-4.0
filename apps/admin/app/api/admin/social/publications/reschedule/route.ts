...
import { rescheduleSocialPublication, socialRescheduleMutationSchema } from "@/lib/admin/social/operations-service";
...
function mappedError(error: unknown) {
  ...
  if (code === "SOCIAL_RESCHEDULE_REQUIRES_FUTURE_TIME") return { error: "future-time-required", status: 409 };
  if (code === "SOCIAL_RESCHEDULE_NOT_ALLOWED") return { error: "reschedule-not-allowed", status: 409 };
  ...
  return { error: "reschedule-failed", status: 500 };
}
...
export async function POST(request: Request) {
  ...
  const parsed = socialRescheduleMutationSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "invalid-request" }, { status: 400 });

  try {
    const result = await rescheduleSocialPublication({ mutation: parsed.data, actor: identity.actor });
