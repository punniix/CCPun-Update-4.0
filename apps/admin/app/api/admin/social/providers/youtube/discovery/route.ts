...
import { fetchYouTubeReadOnlyDiscovery } from "@/lib/admin/social/providers/youtube/read-only";
...
export async function POST(request: Request) {
  ...
  try {
    const discovery = await fetchYouTubeReadOnlyDiscovery();
    return NextResponse.json({ requestId, discovery }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    const code = error instanceof Error ? error.message : "";
    if (code === "YOUTUBE_READ_NOT_CONFIGURED") return NextResponse.json({ error: "provider-not-connected", requestId }, { status: 409 });
    if (code === "YOUTUBE_READ_AUTH_REQUIRED") return NextResponse.json({ error: "provider-auth-required", requestId }, { status: 401 });
    if (code === "YOUTUBE_READ_RATE_LIMITED") return NextResponse.json({ error: "provider-rate-limited", requestId }, { status: 429, headers: { "Retry-After": "60" } });
    if (code === "YOUTUBE_READ_TIMEOUT") return NextResponse.json({ error: "provider-timeout", requestId }, { status: 504 });
    if (code === "YOUTUBE_READ_INVALID_RESPONSE") return NextResponse.json({ error: "provider-invalid-response", requestId }, { status: 502 });
