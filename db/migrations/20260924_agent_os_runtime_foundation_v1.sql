...
CREATE INDEX IF NOT EXISTS agent_runtime_job_active_idx
  ...
  WHERE status IN (
    'queued','running','waiting_external','waiting_ai','validating',
    'awaiting_review','retrying','reconciliation_required'
  );
...
CREATE TABLE IF NOT EXISTS ccpun_admin.agent_runtime_job_event (
  ...
  job_id uuid NOT NULL REFERENCES ccpun_admin.agent_runtime_job(job_id),
...
BEGIN
  ...
  SELECT * INTO v_job
  FROM ccpun_admin.agent_runtime_job
  WHERE job_id=NULLIF(payload->>'job_id','')::uuid
  FOR UPDATE;
  ...
  UPDATE ccpun_admin.agent_runtime_job SET
    ...
    heartbeat_at=CASE WHEN payload ? 'heartbeat_at'
      THEN NULLIF(payload->>'heartbeat_at','')::timestamptz ELSE heartbeat_at END,
    started_at=CASE WHEN payload ? 'started_at'
      THEN NULLIF(payload->>'started_at','')::timestamptz ELSE started_at END,
    completed_at=CASE
    ...
    n8n_execution_id=CASE WHEN payload ? 'n8n_execution_id'
      THEN NULLIF(payload->>'n8n_execution_id','') ELSE n8n_execution_id END,
    provider_reference=CASE WHEN payload ? 'provider_reference'
      THEN NULLIF(payload->>'provider_reference','') ELSE provider_reference END,
    error_category=CASE WHEN payload ? 'error_category'
      THEN NULLIF(payload->>'error_category','') ELSE error_category END,
    ...
    queue_wait_ms=CASE WHEN payload ? 'queue_wait_ms'
      THEN NULLIF(payload->>'queue_wait_ms','')::bigint ELSE queue_wait_ms END,
...
AS $agent_events$
SELECT
  e.event_id,e.job_id,e.row_version,e.status,e.stage,e.attempt,e.error_category,e.occurred_at
...
REVOKE ALL PRIVILEGES ON
  ccpun_admin.agent_runtime_job,
  ccpun_admin.agent_runtime_job_event
...
VALUES('20260924_agent_os_runtime_foundation_v1','sha256:09fe7ab43995ebba81aa914e47fd3b686c989ef1934e414443251d7b8c79cd47')
