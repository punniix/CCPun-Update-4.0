...
test("Drive credential readiness recognizes the intended memory-only owner-interactive setup", () => {
  ...
  const incomplete = getLineProviderActivationReadiness({
    NEXT_PUBLIC_CCPUN_GOOGLE_DRIVE_OAUTH_CLIENT_ID: "client.apps.googleusercontent.com",
  });
...
test("Health shows owner actions and treats Drive as on-demand authorization, not a missing persistent credential", () => {
  ...
  assert.match(action, /เปิด Rich Menu หลัก/);
